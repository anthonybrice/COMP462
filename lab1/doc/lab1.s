...
loop
    BL  Port1_Input                  ; if so, switch 2 pressed
    CMP R0, #0x00                   ; R0 == 0x00?
    ...
    B   loop
bothpressed
    MOV R0, #0x13                  ; R0 = GREEN (green LED on)
    BL  Port1_Output                ; turn the green LED on
    B   loop
nopressed
    MOV R0, #0x12                      ; R0 = 0 (no LEDs on)
    BL  Port1_Output                ; turn all of the LEDs off
    B   loop
...
Port1_Init
    ...
    STRB R0, [R1]
    LDR  R1, =P1DIR
    MOV  R0, #0x01                  ; make P1.4 and P1.1 inputs
    STRB R0, [R1]
    LDR  R1, =P1REN
